...
export class DocumentRegistry implements vscode.FileSystemProvider {
  ...
  closeAll(): void {
    for (const path of [...this.uriByPath.keys()]) {
      this.close(path)
    }
  }
  ...
  async applyPatch(path: string, lnum: number, count: number, lines: string[]): Promise<boolean> {
    ...
    if (count !== -1 && lnum > entry.lines.length) {
      return false
    }

    // Update in-memory lines first
    const endIdx = count === -1 ? entry.lines.length : lnum + count
    entry.lines = [...entry.lines.slice(0, lnum), ...lines, ...entry.lines.slice(endIdx)]
  ...
  private onLocalChange(ev: vscode.TextDocumentChangeEvent): void {
    if (ev.document.uri.scheme !== SCHEME) {
      return
    }
    ...
    if (!this.sendPatch || this.role === 'ro') {
      return
    }
    ...
    for (const change of ev.contentChanges) {
      ...
      entry.lines = [...entry.lines.slice(0, startLine), ...newLines, ...entry.lines.slice(startLine + count)]
    }
  }
